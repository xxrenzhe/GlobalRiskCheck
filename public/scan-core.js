window.__GRC_CONFIG__ = {
  ...
  messages: {
    hosting: "🚨 致命风险：您正在使用廉价机房 IP。大厂特征库已将此类 IP 标记为群控滥用节点，秒封率 >87%！",
    creep:
      "🚨 重度环境伪装：检测到您的浏览器正在尝试发送虚假的底层硬件数据，已被风控 AI 标记为【高危作弊设备】。",
    webrtc: "⚠️ 底层穿透风险：代理软件未能接管 UDP 流量，您的真实物理 IP 已暴露给网页。",
    device:
      "⚠️ 硬件裸奔：您的底层设备特征未做物理级隔离，且系统参数存在逻辑漏洞，易被判定为矩阵强关联。"
  }
};

Object.freeze(window.__GRC_CONFIG__);
